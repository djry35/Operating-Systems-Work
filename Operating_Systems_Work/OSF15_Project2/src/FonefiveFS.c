...
//putting up here because simplicity. 
//Resizes the block(s) referenced by the directory. If the directory has the inode number,
//it will remove it and shift the rest over to close the gap. If the inode number is not in the directory,
//it will add the number to the directory. 
//Inputs:
//	fs: the file system with the directory
//	inode: the directory that we are modifying
//	toChange: the inode number that needs to be added/subtracted from the directory.
//Returns:
//	True or False on success of operation.
//NOTE: DEPENDENT ON DIR_REC_MAX. If DIR_REC_MAX >= BLOCK_SIZE / sizeof(inode_ptr_t), the function
//will have unpredictable results. Will modify/handle if I have time.  
bool restructure_directory(F15FS_t* const fs, inode_t* inode, inode_ptr_t toChange);
...
{
	//param check, of course. 
	if(!fs || !fs->bs || !(fname && fname[0]) || strlen(fname) > FNAME_MAX)
	{
		return -1;
	}
	//getting the parent and child name sectioned off. Thank you Will...
	char* path_copy = malloc(sizeof(char)*FS_PATH_MAX);
	memcpy(path_copy, fname, strlen(fname) + 1);
	char* parentRef = strrchr(path_copy, '/');
	*parentRef = '\0';
	parentRef ++;
	char* parent = malloc(sizeof(char)*FS_PATH_MAX);
	memcpy(parent, path_copy, strlen(path_copy) + 1);
	//if we're at the root, gotta do a little tweaking
	if(*parent == '\0')
	{
		*parent = '/';
		*(parent + 1) = '\0';
	...
	//go find the inode blocks that contain the child and parent.
	//my naming conventions are weird, will change if time allows.
	search_struct_t fileFindings;
	locate_file(fs, parentRef, &fileFindings);
	if(!fileFindings.success || !fileFindings.valid || !fileFindings.data)
	{
		return -2;
	}
	
	search_struct_t parentFindings;
	locate_file(fs, parent, &parentFindings);
	if(!parentFindings.success || !parentFindings.valid || !parentFindings.data)
	{
		return -2;
	}
	//after the blocks are found, load the actual inodes for our use.
	inode_t fileInode;
	inode_t parentInode; 
	if(!load_inode(fs, fileFindings.inode, (void*)&fileInode) || !load_inode(fs, parentFindings.inode, (void*)&parentInode))
	{
		return -3;
	}
	//if the file is a directory, we have to look to see if it's empty.
	if(fileInode.mdata.type == DIRECTORY)
	{
		dir_rec_t dirContents;
		fs_get_dir(fs, path_copy, &dirContents);
		if(dirContents.total != 0)
		{
			//directory not empty! Can write to stderr, oh well.
			return -2;
		}
		else
		{
			//first process I thought of to wipe out the node. 
			inode_t i;
			memset(&i, 0, sizeof(inode_t));
			
			//oh god what have I done...I need to wipe the blocks too. Hopefully I have time.
			if(!write_inode(fs, fileFindings.inode, (void*)&i))
			{
				return -6;
			}
			//after wiping the inode, fix up the parent directory and write the parent, 
			//if applicable. It's setup so that the inode write doesn't really do anything.
			if(!restructure_directory(fs, &parentInode, fileFindings.inode))
			{
				return -5;
			}
			if(!write_inode(fs, parentFindings.inode, (void*)&parentInode))
			{
				return -4;
			}
		}
	}
	//k, not a directory. Just delete it.
	else
	{
		//was seeing if doing stuff in a different order does anything.
		//Verdict: does nothing. 
		if(!restructure_directory(fs, &parentInode, fileFindings.inode))
		{
			return -5;
		}
		//same process, same issues as above
		inode_t i;
		memset(&i, 0, sizeof(inode_t));
		if(!write_inode(fs, fileFindings.inode, (void*)&i))
		{
...
{
	//param check
	if(!fs || !fs->bs || !(fname_src && fname_src[0]) || strlen(fname_src) > FNAME_MAX ||
		!(fname_dst && fname_dst[0]) || strlen(fname_dst) > FNAME_MAX)
	{
		return -1;
	}
	//setup the parent and child filenames, thank you Will again.
	char* path_copy = malloc(sizeof(char)*FS_PATH_MAX);
	memcpy(path_copy, fname_src, strlen(fname_src) + 1);
	char* parentRef = strrchr(path_copy, '/');
	*parentRef = '\0';
	parentRef ++;
	char* parent = malloc(sizeof(char)*FS_PATH_MAX);
	memcpy(parent, path_copy, strlen(path_copy) + 1);
	//tweak for root parent
	if(*parent == '\0')
	{
		*parent = '/';
		*(parent + 1) = '\0';
	}	
	//step 1: find the inode locations for the parent and child.
	search_struct_t fileFindings;
	locate_file(fs, parentRef, &fileFindings);
	if(!fileFindings.success || !fileFindings.valid || !fileFindings.data)
	{
	...
	//go ahead and look for the destination file as well. I shouldn't be able to find it.
	search_struct_t dstFileFindings;
	locate_file(fs, fname_dst, &dstFileFindings);
	if(dstFileFindings.success && dstFileFindings.valid)
	{
		//Wait. This isn't the right way to do it. TODO: fix this.
		return -7;
	}		


	//here's the actual place to set up the parent and child filenames 
	char* newParentRef = strrchr(fname_dst, '/');
	*newParentRef = '\0';
	newParentRef ++;
	char* newParent = malloc(sizeof(char)*FS_PATH_MAX);
	
	//go ahead and clear the old filename out.
	memset(fileInode.fname, 0, strlen(fileInode.fname));


	//ok, so there's 2 scenarios:
	//	the file is moving out of its parent dir
	//	the file is just getting renamed
	
	//this is if the file isn't moving out of the directory. Super easy.
	if(strcmp(newParent, parent) == 0)
	{
		//just rename the child, and write the update back to the inode.
		memcpy(fileInode.fname, fname_dst, strlen(fname_dst));
		if(!write_inode(fs, fileFindings.inode, (void*)&fileInode))
		{
			return -4;
		}
	}
	...
	{
		//hmm, okay. So first, give the child the new name. 
		memcpy(fileInode.fname, fname_dst, strlen(fname_dst));
		//now we find the parent directory. We should be able to find it.
		search_struct_t newParentFindings;
		locate_file(fs, newParent, &newParentFindings);
		if(!newParentFindings.success || !newParentFindings.valid || !newParentFindings.data)
		{
			return -2;
		}
		//Load the new parent inode to modify.
		inode_t newParentInode;
		if(!load_inode(fs, newParentFindings.inode, (void*)&newParentInode))
		{
			return -3;
		}
		//1. Get the file out of the old directory.
		//2. Get the file into the new directory.
		//3. Write both out to the file system.
		//Well, not structured in that order, but yeah.
		if(!restructure_directory(fs, &parentInode, fileFindings.inode))
		{
			return -5;
		}
		...
		if(!restructure_directory(fs, &newParentInode, fileFindings.inode))
		{
			return -5;
		}
		if(!write_inode(fs, parentFindings.inode, (void*)&parentInode))
...
bool restructure_directory(F15FS_t* const fs, inode_t* inode, inode_ptr_t toChange)
{
	//So I just realized that since there are 20 files max in a dir, everything's in the first block pointer. I'm just gonna cheat this. Will improve maybe, sometime in the future. 
	dir_block_t data;
	if(!load_block(fs, inode->data_ptrs[0], (void*)&data))
	{
		return false;
	}
	//k, so here we go through each file in the directory.
	for(int i = 0; i < 20; i++)
	{
		//We found a match. This means that the file is being moved out of the directory, 
		//so we need to get rid of it.
		if(data.entries[i].inode == toChange)
		{
			//here is where you could defragment, that takes effort though. 
			//I'm going to assume this is good enough, but we'll see.
			//But shouldn't this be a memset to clear the whole entry? Ugh. 
			data.entries[i].inode = 0;
			//write the update back to the inode, and done.	
			return write_block(fs, inode->data_ptrs[0], (void*)&data);
		}
	}
	//I will assume that if we are removing a node, we will find it by now. 
	//So at this point, we are adding this number to the directory. 
	for(int i = 0; i < 20; i++)
	{
		//k, so the entry we found is already in use, find another one.
		if(data.entries[i].inode > 0)
		{
			continue;
		}
		else
		{
			//found an open slot! Save the file inode in the directory
			//and write back to file. 
			data.entries[i].inode = toChange;
			return write_block(fs, inode->data_ptrs[0], (void*)&data);
		}
	}
	//THIS SHOULD NEVER HAPPEN. 
	//Wait, this will happen if the directory is full. How to handle...hmm...
	return false;
}
